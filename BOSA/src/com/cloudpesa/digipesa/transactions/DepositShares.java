 ...
 * @author Festus
 */
public class DepositShares 
{
    public void sharesScreen() 
    {
        showScreen("Deposit to Shares", createDepositScreen());
    }
            ...
            public void actionPerformed(ActionEvent evt) {
                if(txt_Amt.getText().equals("")){
                    ...
                    if(Double.parseDouble(Amount) < 1){
                        ...
                        try {transact("Shares Capital", Amount.toString());} catch (IOException ex) {}
                    }
                }
            }
        });
        
        Container demoContainer = BorderLayout.center(BoxLayout.encloseY(new SpanLabel("Enter Amount", "FlagButton"), txt_Amt));
        ...
        if (dataa != null) 
        {
            if(request.getResponseCode() == 200 || request.getResponseCode() == 201)
            {
                ...
                if (response.contains("\"Message\"")) {
                    // Handle the case where the "Message" key is present in the JSON response
                    System.out.println("The 'Message' key is present in the JSON response.");
